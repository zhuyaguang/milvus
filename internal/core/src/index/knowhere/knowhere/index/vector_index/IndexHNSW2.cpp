...
BinarySet
IndexHNSW2::Serialize(const Config& config) {
    if (!index_) {
    ...
    try {
        ...
        BinarySet res_set;
        res_set.Append("HNSW2", data, writer.rp);
...
void
IndexHNSW2::Load(const BinarySet& index_binary) {
    try {
        Assemble(const_cast<BinarySet&>(index_binary));
        auto binary = index_binary.GetByName("HNSW2");
...
void
IndexHNSW2::Train(const DatasetPtr& dataset_ptr, const Config& config) {
    try {
...
void
IndexHNSW2::AddWithoutIds(const DatasetPtr& dataset_ptr, const Config& config) {
    if (!index_) {
...
DatasetPtr
IndexHNSW2::Query(const DatasetPtr& dataset_ptr, const Config& config, const faiss::BitsetView bitset) {
    if (!index_) {
        KNOWHERE_THROW_MSG("index not initialize or trained");
    }
